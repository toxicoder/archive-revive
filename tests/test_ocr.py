...
class TestRunOCR(unittest.TestCase):
    ...
    @patch('pytesseract.image_to_alto_xml', side_effect=pytesseract.TesseractNotFoundError)
    def test_run_ocr_tesseract_not_found(self, mock_image_to_alto_xml):
        """Test that run_ocr raises TesseractNotFoundError when Tesseract is not found."""
        with self.assertRaises(pytesseract.TesseractNotFoundError):
            run_ocr(self.image_path, self.output_dir, 3)

    @patch('pytesseract.image_to_alto_xml', side_effect=Exception("Test error"))
