...
class TestNormalizeRagErrors(unittest.TestCase):
    ...
    def test_generate_rag_json_file_not_found(self):
        """
        Test that generate_rag_json returns False when the ALTO file is
        not found.
        """
        result = generate_rag_json(
            "non_existent_file.xml", self.output_json_path, self.config
        )
    ...
    @patch('lxml.etree.fromstring', side_effect=etree.XMLSyntaxError(
        "Test error", 1, 1, 1
    ))
